...
> 平台即服務 (PaaS) 是一種計算模型，<mark>提供基於雲端的平台來開發、運行和管理應用程式。</mark>PaaS 為開發人員提供了一個框架，他們可以利用這個框架來建立和使用自訂應用程式。PaaS之所以吸引人，是因為它為開發人員提供了一個平台，讓他們可以開發、運行和管理自己的應用程式，而無需建立和維護相關的基礎架構。他們還可以利用內建的軟體組件來構建應用程式，從而減少需要編寫的程式碼量。Google Cloud 的 **Cloud Run** 和 **BigQuery** 就是 PaaS 產品的例子。Cloud Run 是一個**完全託管的無伺服器平台**，用於大規模開發和託管應用程式，它負責根據需求配置伺服器和調整應用程式執行個體的規模。BigQuery 是一個**完全託管的企業級數據倉庫**，用於管理和分析數據，並且可以通過查詢來回答大數據問題，而無需任何基礎架構管理。那麼，PaaS 的好處有哪些呢？它可以**減少開發時間**。開發人員可以直接進行編碼，而不必花費時間設置和維護開發環境，從而縮短上市時間。它具有**可擴展性**。借助 PaaS，組織可以根據需要購買額外的容量來構建、測試、階段和運行應用程式。它還允許應用程式設計師利用雲基礎架構固有的可擴展性。它可以**減少管理工作**。通過進一步抽象底層資源的管理（比 IaaS 更進一步），PaaS 將基礎架構管理、修補程式、更新和其他管理任務分擔給雲服務提供商。這提供了一種**經濟有效**的方法來專注於新功能。而且它很**靈活**。通過支援不同的程式語言和方便分佈式團隊的協作，PaaS 為開發人員提供了靈活性，使他們能夠在同一個平台上交付各種項目，從原型到企業解決方案。那麼，PaaS 適用於哪些場景呢？PaaS 適用於以下組織：想要創建獨特且自訂的應用程式，而無需大量投資於擁有和管理基礎架構；想要**快速測試和佈署應用程式**；擁有許多老舊應用程式，並想要**降低營運成本**；有一個新的應用程式項目，他們希望通過盡可能快地擴展和更新應用程式來快速佈署；只想在使用資源時才支付費用；想要將耗時的任務（例如設置和維護應用程式伺服器以及開發和測試環境）外包。
...
> Vertex AI 也是建立自訂端到端機器學習模型的重要平台。這意味著不僅可以使用您自己的數據訓練模型，而且模型也是自訂建立的。Vertex AI 提供一套產品，可在機器學習工作流程的每個階段提供幫助，從收集數據到未來的工程、建立模型，最後到佈署和監控這些模型。由於這種方法是完全自訂建立的，端到端的，因此它的過程最長，需要一個專業的數據科學家和工程師團隊。但是，這些完全自訂的機器學習模型最能滿足您的需求，並為您的業務帶來最大的差異化和創新成果。Vertex AI 包含可協助程式設計師進行虛擬機器映像、數據標記、訓練和預測的工具。它還提供預建演算法。重要的是要記住，儘管這些工具是每個階段使用您的數據的構建塊，但沒有任何一種方法適合所有情況。每個用例都需要不同的工具和產品組合。
