...
> 歷年來，Google 投資數十億美元打造其網絡，使其成為全球最大的網絡之一。該網絡旨在為客戶應用程式提供最高的吞吐量和最低的延遲。Google Cloud 的基礎架構遍佈五大主要地理位置：北美洲、南美洲、歐洲、亞洲和澳洲。擁有眾多服務據點至關重要，因為應用程式的佈署位置會影響可用性、持久性和延遲等品質，其中延遲是指資訊封包從源頭傳輸到目的地的時間。每個地理位置都劃分為數個不同的區域和區域。區域代表獨立的地理區域，並由區域組成。例如，倫敦 (europe-west2) 是一個目前包含三個不同區域的區域。區域是佈署 Google Cloud 資源的地區。例如，如果您使用 Compute Engine 啟動虛擬機器，它將在您指定的區域中執行，以確保資源冗餘。您可以在不同的區域中運行資源。這對於將應用程式更靠近全球各地的用戶，以及在整個區域出現問題（例如自然災害）時提供保護非常有用。部分 Google Cloud 服務支援將資源放置在我們稱為多區域的位置。例如，Cloud Storage 允許您將數據放置在歐洲多區域中。這表示數據至少冗餘儲存在兩個地理位置，在歐洲境內相隔至少 160 公里，例如倫敦和比利時。您可以在 cloud.google.com/about/locations 找到 Google Cloud 區域和區域的最新數據。
...
*   大多數組織使用多種雲端模式。
*   抽象化降低雲端運算的複雜性。
*   本地佈署如擁車，IaaS如租車，PaaS如搭計程車，SaaS如搭公車。
