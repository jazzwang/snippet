...
> 理解雲端如何改變企業，學習雲端和雲端技術的基礎知識至關重要。在本課程單元中，您將探討：與雲端和數位轉型相關的重要術語、雲端技術於組織數位轉型之優勢、<mark>自建機房（On-premises infrastructure）、公有雲（Public Cloud）、私有雲（Private Cloud）、混合雲（Hybrid Cloud）和多雲（Multi-Cloud）</mark>之間的差異，以及促使組織進行數位轉型的驅動因素和挑戰。讓我們開始吧！
...
> 雲端運算相較於傳統本地基礎架構的優勢有哪些呢？它具有**可擴展性(scalable)**。雲端運算讓組織可以按需取得可擴展的資源和最新的技術，無需擔心資本支出或有限的固定基礎架構。這可以大幅加快基礎架構佈署時間。它很**靈活(flexible)**。組織及其使用者可以從任何地方存取雲端服務，並根據需要擴展或縮減服務以滿足業務需求。它很**敏捷**。組織可以開發新的應用程式並快速投入生產，無需擔心底層基礎架構。它具有**戰略價值**。因為雲端供應商始終更新最新的創新技術並將其作為服務提供給客戶，所以組織可以獲得更多競爭優勢和更高的投資回報，勝過投資即將淘汰的技術。這讓組織可以更快地創新和嘗試新想法。它很**安全**。由於雲端供應商所實施的安全機制深度和廣度以及專職團隊，雲端運算的安全性能被認為優於企業數據中心。最後，它具有**成本效益**。無論組織實施哪種雲端運算服務模式，他們只需支付使用的運算資源費用。他們無需超量建置數據中心容量以應付需求或業務增長的突然飆升，並且可以佈署 IT 人員來處理更具策略性的計劃。
...
( Jazz 註：保留 Gemini LLM 精簡後的說明翻譯，因為我在 prompt 中請 LLM 在產生摘要時，盡量精簡扼要。看樣子第二個摘要任務影響到第一個翻譯任務。挺有趣的～ )
...
> 加速創新的組織，其思維方式的一個主要指標在於它們如何看待轉型。它們不再詢問應用程式和服務應在哪裡執行的基礎架構問題，而是詢問<mark>如何建立一個能幫助每個人、每個流程和每項技術適應不斷變化業務需求的環境</mark>問題。那麼，究竟是什麼類型問題促使組織進行數位轉型？在 Google，當我們與客戶討論他們**最大的商務挑戰**以及他們需要**加速數位轉型**的因素時，我們始終聽到**五個主題**。首先，他們希望**成為理解和使用數據的佼佼者**。如今，組織必須<mark>整合來自**資料流(Stream)、資料湖泊(Lake)、資料倉儲(Warehouses)和資料庫(Batabase)**的數據，以便他們能夠快速輕鬆地**打破數據孤島(Data Silo)**，生成**即時洞察(Real-time Insights)**，並做出更好的業務決策；從而降低成本和提高效率。</mark>其次，他們希望擁有**最佳的技術基礎架構**。組織正在尋找一個雲平台作為其發展的基礎，並且該平台具有**靈活性**和**安全性**，能夠根據市場需求快速適應創新。第三，他們希望**創造最佳的混合工作場所**。工作方式和地點的根本性轉變需要新的、更強大的聯繫和協作，許多以前當面進行的互動已經數位化。這種變化需要更刻意地建立聯繫和協作。第四，對於組織而言，至關重要的是要**知道他們的數據、系統和用戶是安全的**。數位世界正在面臨更嚴重的安全問題，因此現在公司正在重新考慮其安全態勢。他們必須找到方法來識別和保護從人員客戶到數據和交易的一切，以應對瞬息萬變的環境。最後，組織將**永續性**作為一個關鍵的、董事會級別的話題來優先考慮。他們希望通過最大限度地減少環境影響的產品和服務來創造更永續的未來。這些就是我們看到的**數位轉型的主要驅動因素**，以及許多組織在數位轉型過程中面臨的挑戰。
...
> 本課程第二單元：Google Cloud 數位轉型到此結束。讓我們快速回顧一些雲端基本概念。我們探討了不同產業客戶如何透過遷移至雲端，創造新的工作方式，從而轉型其業務的案例。雲端的總擁有成本計算與傳統的靜態長期採購模式大相逕庭，雲端更具**動態性**和**按需使用**特性。遷移至雲端意味著將支出從**資本支出**轉變為**營運支出**模式，使組織只需為其使用的資源和使用的時間付費。我們定義了**私有雲**、**混合雲**和**多雲**，並描述了促使組織選擇這些方法的不同業務驅動因素。最後，您學習了**快速、可靠且低延遲**的全球網路作為轉型和卓越用戶體驗的基礎的重要性。
...
When moving to the cloud, there are decisions to make about how to manage and operate different cloud services. One of those decisions is around the type of cloud computing service model to use. Organizations typically choose **service model types** based on their specific business requirements. In this section of the course, you'll explore three main cloud computing service models: (1) IaaS, or **Infrastructure as a Service** (2) PaaS, or **Platform as a Service** and (3) SaaS, or **Software as a Service**. Because the levels of responsibility between an organization and their cloud service provider vary depending on which model is used, you'll also examine the **shared responsibility model** between Google Cloud and our customers. By the end of this section, you'll be able to: (1) Define IaaS, PaaS, and SaaS. (2) Compare and contrast the benefits and tradeoffs of IaaS, PaaS, and SaaS. (3) Determine which computing model applies to various business scenarios and use cases. (4) Describe the cloud shared responsibility model. And (5) identify which responsibilities are the cloud provider's or the customer's for on-premises and cloud computing models.

> 遷移到雲端時，需要決定如何管理和操作不同的雲端服務。其中一個決定是關於要使用的雲端運算服務模式類型。組織通常會根據其特定的業務需求選擇**服務模式類型**。在本課程的這一部分，您將探討三種主要的雲端運算服務模式：(1) IaaS（**基礎設施即服務**）、(2) PaaS（**平台即服務**）和 (3) SaaS（**軟體即服務**）。由於組織和其雲端服務提供者之間的責任等級會因應使用的模式而有所不同，您還將檢查 Google Cloud 與我們的客戶之間的**共享責任模式**。在本節結束時，您將能夠 (1) 定義 IaaS、PaaS 和 SaaS；(2) 比較和對比 IaaS、PaaS 和 SaaS 的優點和權衡；(3) 確定哪些運算模型適用於各種業務情境和用例；(4) 描述雲端共享責任模式；以及 (5) 識別哪些責任屬於雲端提供者或客戶的，適用於本地和雲端運算模式。
...
> 平台即服務 (PaaS) 是一種計算模型，<mark>提供基於雲端的平台來開發、運行和管理應用程式。</mark>PaaS 為開發人員提供了一個框架，他們可以利用這個框架來建立和使用自訂應用程式。PaaS之所以吸引人，是因為它為開發人員提供了一個平台，讓他們可以開發、運行和管理自己的應用程式，而無需建立和維護相關的基礎架構。他們還可以利用內建的軟體組件來構建應用程式，從而減少需要編寫的程式碼量。Google Cloud 的 **Cloud Run** 和 **BigQuery** 就是 PaaS 產品的例子。Cloud Run 是一個**完全託管的無伺服器平台**，用於大規模開發和託管應用程式，它負責根據需求配置伺服器和調整應用程式執行個體的規模。BigQuery 是一個**完全託管的企業級資料倉儲**，用於管理和分析數據，並且可以通過查詢來回答大數據問題，而無需任何基礎架構管理。那麼，PaaS 的好處有哪些呢？它可以**減少開發時間**。開發人員可以直接進行編碼，而不必花費時間設置和維護開發環境，從而縮短上市時間。它具有**可擴展性**。借助 PaaS，組織可以根據需要購買額外的容量來構建、測試、階段和運行應用程式。它還允許應用程式設計師利用雲基礎架構固有的可擴展性。它可以**減少管理工作**。通過進一步抽象底層資源的管理（比 IaaS 更進一步），PaaS 將基礎架構管理、修補程式、更新和其他管理任務分擔給雲服務提供商。這提供了一種**經濟有效**的方法來專注於新功能。而且它很**靈活**。通過支援不同的程式語言和方便分佈式團隊的協作，PaaS 為開發人員提供了靈活性，使他們能夠在同一個平台上交付各種項目，從原型到企業解決方案。那麼，PaaS 適用於哪些場景呢？PaaS 適用於以下組織：想要創建獨特且自訂的應用程式，而無需大量投資於擁有和管理基礎架構；想要**快速測試和佈署應用程式**；擁有許多老舊應用程式，並想要**降低營運成本**；有一個新的應用程式項目，他們希望通過盡可能快地擴展和更新應用程式來快速佈署；只想在使用資源時才支付費用；想要將耗時的任務（例如設置和維護應用程式伺服器以及開發和測試環境）外包。
...
* 減少管理負擔，將基礎設施管理外包。
* 靈活支援多種程式語言及團隊協作。
* 適用於需要快速開發、佈署及彈性擴展的應用程式。
* 適合希望降低基礎設施投資和營運成本的組織。
* 適用於擁有大量老舊應用程式並希望降低維護成本的組織。
...
> 組織如何決定哪種雲端運算模式最適合自身？答案<mark>取決於其業務需求、所需功能和可用專業知識。</mark>如果他們正在尋找高度靈活、可擴展的服務，同時又能控制其基礎架構，那麼 IaaS 是正確的選擇。此模型提供**最大的控制和自定義選項**，但也需要最多的管理責任和技術專業知識。如果他們需要一個設計用於構建軟體產品的平台，那麼 PaaS 將立即幫助他們的業務。這提供了一種經濟高效的構建應用程式的方法，但仍然需要一些技術專業知識和較少的管理工作。如果他們想要即用型功能，而無需安裝的麻煩，那麼 SaaS 可能是最佳選擇。這代表最少的管理責任和技術專業知識，但也提供最少的控制和自定義選項。然而，這些運算模式並非相互排斥的。根據用例的不同，大多數組織將結合使用所有三種模式來滿足不同的業務需求。他們需要根據**管理級別、控制、責任、靈活性**以及所需的**專業知識**等變量來比較他們的選項。例如，想像一下，一個大型組織需要實施一個新的庫存管理系統。如果他們擁有內部開發專業知識並願意管理基礎架構，他們可以使用 IaaS 資源來構建它。組織的 IT 團隊將完全控制伺服器配置，但也需要承擔管理和維護的負擔。他們可以選擇 PaaS 解決方案並構建自定義 CRM 應用程式，同時將基礎架構的管理負擔轉嫁給雲服務提供商；保留對應用程式功能的完全控制，但減少管理負擔。最後，他們可以選擇購買現成的 SaaS 解決方案；無需每日管理基礎架構，但也放棄對軟體中所有功能和功能的控制。這些選項都是可行的解決方案，因此組織必須比較每個用例的優缺點。這些雲端運算服務模式為組織提供了本地託管無法提供的選擇、靈活性和選項。
...
One area of responsibility where each of the cloud computing models differ is **security**. When an organization manages its data in its own data centers, that organization is responsible for all aspects of its security. However, as infrastructure is moved to the cloud, some aspects of the responsibility shift to the cloud provider. This concept is called the **shared responsibility model**. <mark>Security in the cloud is a shared responsibility between the cloud provider and the customer.</mark> Although direct responsibilities change based on the cloud computing service model, <mark>organizations are always in control of securing their data<m/ark>, and the <mark>cloud provider is always responsible for securing the infrastructure.</mark> At Google Cloud, we defend organizations' data against threats and fraudulent activity with the same infrastructure and security services we use for our own operations. However, security of the cloud and security in the cloud are two different things. Simply put, the cloud provider is responsible for the **security of the cloud**, while the customer is responsible for **security in the cloud**. It's important for organizations to understand how the specific customer responsibilities vary according to the type of cloud computing model used. This is especially important because, according to a Gartner report, 99% of all cloud security failures will result from user error through the year 2025. Organizations must understand their roles and responsibilities in cloud security to guarantee it.

> 雲端運算模型在**安全**責任方面有所不同。當組織自行管理數據中心時，需負責所有安全層面。然而，隨著基礎設施遷移至雲端，部分責任會轉移給雲端供應商，此概念稱為**共享責任模型**。<mark>雲端安全是雲端供應商和客戶共同的責任。</mark>儘管直接責任會根據雲端運算服務模型而有所不同，但組織始終控制其數據安全，而雲端供應商始終負責基礎設施安全。在 Google Cloud，我們使用與自身營運相同的基礎設施和安全服務來保護組織的數據免受威脅和欺詐活動。然而，雲端安全與雲端內的安全是兩碼事。簡而言之，雲端供應商負責雲端安全，客戶負責雲端內的安全。組織務必理解特定客戶責任如何根據所使用的雲端運算模型類型而有所不同。這一點尤其重要，因為根據 Gartner 的報告，到 2025 年，99% 的雲端安全故障將是由使用者錯誤造成。組織必須理解其在雲端安全中的角色和責任才能確保安全。
...
> 本課程已完成 Google Cloud 數位轉型課程的第三單元。讓我們快速回顧一下雲端運算模型和共享責任。您學習了三種主要的雲端運算服務模型：IaaS（基礎設施即服務）、PaaS（平台即服務）和 SaaS（軟體即服務）。每個模型都提供不同層級的服務和產品組合，以滿足組織的需求。您還學習了每種雲端運算模型的主要優點和權衡取捨，如何幫助組織選擇最符合其業務需求的模型。透過理解每種運算模型的內容，組織將能更好地做出正確的資源和預算決策。最後，您學習了雲端供應商及其客戶之間的共享責任。雖然雲端供應商將確保雲端基礎設施的安全，但保護其資料的安全則是客戶的責任。
...
> 本課程「以 Google Cloud 實現數位轉型」到此結束，您已學習雲端科技的基礎知識，以及此科技如何改變數位時代的商業模式。課程的第一部分「為何雲端科技正在改變商業模式」，您學習到：與雲端和數位轉型相關的重要術語；關於組織數位轉型的雲端科技優勢；內部佈署基礎架構、公有雲、私有雲、混合雲和多雲之間的差異；以及促使組織進行數位轉型的驅動因素和挑戰。在課程的第二部分「雲端基本概念」中，您學習到：透過客戶業務案例理解遷移到雲端基礎架構的優勢；Google Cloud 中解決方案和產品之間的差異；遷移到雲端如何將組織的支出從資本支出轉變為營運支出，以及這如何影響其總體擁有成本；私有雲、混合雲或多雲基礎架構最適用於哪些不同的業務案例；基本的網路基礎架構術語；以及 Google Cloud 如何透過快速可靠的網路連接的全球基礎架構和資料中心支援數位轉型。在課程的第三部分「雲端運算模型和共享責任」中，您學習到：IaaS、PaaS 和 SaaS 的定義、優勢和權衡；哪種運算模型適用於各種業務情境和案例；雲端共享責任模型是什麼；以及對於內部佈署和雲端運算模型，哪些責任屬於雲端供應商或客戶。現在您已全面理解數位轉型，請繼續修習系列中的下一門課程「探索以 Google Cloud 實現資料轉型」，您將學習到：資料的價值；雲端資料轉型歷程；Google Cloud 資料管理解決方案；以及 Google Cloud 智能分析和商業智慧解決方案。
...
> 商業數據並非新名詞，因為組織長期以來都運用有關績效和營運的資訊來做決策。然而，傳統方法的數據分析可能需要數天或數月，而且往往不完整。此外，通常需要專業團隊才能製作複雜的報告。有了雲端技術，情況就大不相同了。現在可以前所未有的速度和規模來使用、分析和應用數據。事實上，組織現在可以利用雲端技術即時提取數據，來訓練機器學習模型，並採取有利於其業務的方式行動。您不再需要成為資料科學家或技術專家就能執行數據分析。有鑑於此，「探索 Google Cloud 的數據轉換」這門課程旨在幫助您理解數據的價值及其如何影響客戶體驗，學習可用的不同 Google Cloud 資料管理解決方案，並探索 Google Cloud 產品如何使數據對員工更有用、更容易取得。在整個課程中，您將會遇到分級的知識評估。您必須通過這些評估才能獲得課程學分。好的，讓我們開始吧！
...
* 降低數據分析門檻，無需專業技術背景。
* 課程目標：理解數據價值、Google Cloud 資料管理方案及數據應用。
* 課程包含分級評估，需通過以獲得學分。
...
> 數據在組織營運中扮演著不可或缺的角色。因此，有效地儲存和管理數據至關重要。Google Cloud 提供廣泛的資料管理產品和解決方案，每種方案都適用於不同的業務用例。在本課程章節中，您將探討：Google Cloud 資料管理選項及其差異；Cloud Storage 提供的不同儲存類型；如何選擇符合組織需求的正確儲存產品；以及組織如何在雲端遷移和／或現代化其目前的資料庫。


##### 摘要

* Google Cloud 資料管理選項及其差異
...
> 上課內容已介紹雲端儲存空間如何儲存非結構化數據，現在讓我們來探討一些適用於儲存結構化數據的 Google Cloud 數據儲存產品。結構化數據由以預定義格式組織的數字和值組成，可在關聯式資料庫中輕鬆搜尋。課程前面提到，關聯式資料庫將資訊儲存在具有明確定義架構的表格、列和欄中，該架構代表資料庫的結構或邏輯配置。Cloud SQL 提供完全託管的關聯式資料庫，包括 MySQL、PostgreSQL 和 SQL Server 作為服務。其設計目的是將單調但必要的且通常耗時的任務轉移給 Google，例如應用程式修補程式和更新、管理備份和配置複製，讓您可以專注於構建出色的應用程式。全球數千家大型企業都信賴 Cloud SQL，使用 Cloud SQL 的組織可以獲得各種好處。它不需要任何軟體安裝或維護。它支援託管備份，因此備份的數據可以安全地儲存，如果需要還原，則可以訪問。它在 Google 的內部網路以及儲存在資料庫表格、臨時檔案和備份中的數據都會進行加密。並且它包含一個網路防火牆，用於控制對每個資料庫實例的網路訪問。Spanner 是一種完全託管的、關鍵任務的關聯式資料庫服務，可以水平擴展以處理意外的業務高峰。Spanner 經 Google 自身的關鍵任務應用程式和服務的考驗，是支援 Google 數十億美元業務的服務。Spanner 特別適用於需要具有聯接和輔助索引的 SQL 關聯式資料庫管理系統、內建高可用性（提供數據冗餘以減少區域或實例不可用時的停機時間，目標是防止單點故障）、強一致性（確保所有儲存數據的位置都能快速更新到最新的數據版本）和高輸入/輸出操作每秒 (每秒數萬次讀寫或更多) 的應用程式。Cloud SQL 和 Spanner 都是完全託管的資料庫服務，但它們有什麼區別呢？Cloud SQL 是一種適用於 MySQL、PostgreSQL 和 SQL Server 的完全託管的關聯式資料庫服務，可用性超過 99.95%。資料庫遷移服務 (DMS) 可讓您輕鬆地將生產資料庫遷移到 Cloud SQL，同時最大限度地減少停機時間。然後是 Spanner，它是一個完全託管的關聯式資料庫，具有無限擴展性、強一致性和高達 99.999% 的可用性，且計劃維護和架構更改均無停機時間。這個全球分佈式、符合 ACID 的雲端資料庫會自動處理複本、分片和交易處理，因此您可以快速擴展以滿足任何使用模式，並確保產品的成功。在考慮哪個選項最適合您的業務時，請考慮以下情況：如果您已超出任何關聯式資料庫的容量、正在為高吞吐量效能而對資料庫進行分片、需要交易一致性、全球數據和強一致性，或者只是想整合資料庫，請考慮使用 Spanner。如果您不需要水平擴展或全球可用的系統，Cloud SQL 是一個經濟高效的解決方案。我們將探討的最後一個結構化數據儲存解決方案是 BigQuery。BigQuery 是一個完全託管的資料倉儲。正如我們已經理解到的，資料倉儲是一個大型儲存庫，其中包含從組織內部各種來源收集的 PB 級數據，用於指導管理決策。由於它是完全託管的，因此 BigQuery 會處理底層基礎架構，因此用戶可以專注於使用 SQL 查詢來解答業務問題，而無需擔心佈署、可擴展性和安全性。BigQuery 一體提供兩種服務：儲存和分析。它是一個儲存 PB 級數據的地方。作為參考，1 PB 等於 11,000 部 4k 品質的電影。BigQuery 也是分析數據的地方，具有內建功能，例如機器學習、地理空間分析和商業智慧。BigQuery 中的數據在靜止狀態下默認加密，無需用戶執行任何操作。靜態加密是用於保護儲存在磁碟（包括固態硬碟）或備份媒體上的數據的加密。BigQuery 提供與現有合作夥伴生態系統的無縫整合。企業可以利用我們的系統整合商和數據整合合作夥伴生態系統來幫助增強分析和報告。這些整合意味著 BigQuery 可以讓組織充分利用其在商業智慧、數據導入和數據整合工具方面的既有投資。行業研究表明，90% 的組織都採用多雲策略，這增加了數據整合、協調和治理的複雜性。BigQuery 在多雲環境中工作，允許數據團隊通過使用 BigQuery 安全且經濟高效地在多個雲提供商中分析數據來消除數據孤島。BigQuery 還具有內建的機器學習功能，因此可以使用 SQL 直接在 BigQuery 中編寫 ML 模型。如果使用其他專業工具（例如 Google Cloud 的 Vertex AI）來訓練 ML 模型，則可以將數據集直接從 BigQuery 匯出到 Vertex AI，從而實現數據到 AI 生命週期的無縫整合。


##### 摘要

* **Cloud SQL:** 完全託管的關聯式資料庫服務 (MySQL, PostgreSQL, SQL Server)，可用性高，易於遷移，成本效益高，適合不需要水平擴展或全球可用性的應用。
* **Spanner:** 完全託管的、關鍵任務的關聯式資料庫服務，無限擴展性，強一致性，高可用性 (99.999%)，適合需要高吞吐量、全球數據和強一致性的應用。
* **BigQuery:** 完全託管的資料倉儲，提供 PB 級數據儲存和分析能力，整合機器學習、地理空間分析和商業智慧功能，支援多雲環境。
...
> 半結構化數據兼具結構化和非結構化數據的元素。它具有一些定義或一致的特性，但通常不像關聯式資料庫那樣遵循嚴格的結構。半結構化數據更容易組織，因為它通常包含一些組織屬性，例如標籤或元數據。電子郵件訊息就是半結構化數據的一個例子。雖然電子郵件的實際內容是非結構化的，但它確實包含結構化數據，例如發件人和收件人的姓名和電子郵件地址、發送時間等等。Google Cloud 提供兩種半結構化資料儲存產品，Firestore 和 Bigtable。Firestore 是一個靈活、可水平擴展的 NoSQL 雲資料庫，用於實時存儲和同步數據。Firestore 可以被移動和 Web 應用程序直接訪問。Firestore 以文檔的形式存儲數據，文檔存儲在集合中。文檔支持各種數據類型，例如嵌套對象、數字和字符串。Firestore 的主要功能之一是自動縮放。它被設計為根據用戶需求自動縮放，但無論資料庫大小如何，都能保持相同的性能水平。Firestore 還通過用戶設備上的綜合資料庫提供離線使用。離線數據訪問確保應用程序即使在用戶斷開網際網路連接的情況下也能不間斷運行。然後是 Bigtable，Google 的 NoSQL 大數據資料庫服務。它是支持許多核心 Google 服務（包括搜索、分析、地圖和 Gmail）的同一資料庫。Bigtable 旨在以一致的低延遲處理大型工作負載，這意味著 Bigtable 對請求的響應速度很快，並且吞吐量很高，這意味著它可以發送和接收大量數據。因此，它是運營和分析應用程序（包括物聯網、用戶分析和金融數據分析）的絕佳選擇。在決定存儲選項時，如果您處理的半結構化或結構化數據超過 1 TB、數據快速且吞吐量高、數據變化迅速、您正在處理 NoSQL 數據、數據是時間序列或具有自然順序、您正在處理大數據並對數據運行批處理或實時處理，或者您正在對數據運行機器學習算法，則您可以選擇 Bigtable。
...
* Firestore：NoSQL 雲端資料庫，支援實時數據同步，自動擴展，提供離線存取功能。
* Bigtable：NoSQL 大數據資料庫，適用於高吞吐量、低延遲的大型工作負載，例如物聯網數據分析。
...
> 本課程「探索 Google Cloud 的數據轉換」到此結束。讓我們快速回顧一下。在課程的第一部分「數據的價值」中，您學習了數據如何產生商業洞察並推動決策；基本的資料管理概念，例如資料庫、資料倉儲和資料湖泊；組織如何利用其現有數據、收集新數據和外部數據來源來創造價值；雲端如何釋放所有類型數據（包括結構化數據和以前未開發的非結構化數據）的商業價值；關於數據價值鏈，從數據的初始創建到數據激活；以及數據治理在成功的數據旅程中所扮演的重要角色。在課程的第二部分「Google Cloud 資料管理解決方案」中，您學習了 Google Cloud 資料管理選項及其區別；關於 Cloud Storage 提供的不同存儲類別；如何選擇合適的存儲產品以滿足組織的需求；以及組織如何在雲端遷移和/或現代化其現有資料庫。最後，在課程的第三部分「使數據有用且易於訪問」中，您學習了 Looker 如何讓員工輕鬆訪問其所需的數據；實時串流分析如何使數據更有用；以及兩個使數據管道現代化的 Google Cloud 產品：Pub/Sub 和 Dataflow。現在您已經全面理解了數據轉換，請繼續學習系列中的下一門課程「利用 Google Cloud 人工智能創新」，您將學習人工智能和機器學習的基礎知識、選擇 Google Cloud AI 解決方案以及構建和使用 Google Cloud AI 解決方案。下次見！


##### 摘要

* **數據的價值:** 數據驅動決策、資料管理概念 (資料庫、資料倉儲、資料湖泊)、數據價值創造、雲端數據價值解鎖、數據價值鏈、數據治理。
* **Google Cloud 資料管理解決方案:** Google Cloud 資料管理選項、Cloud Storage 存儲類別、存儲產品選擇、資料庫遷移和現代化。
...
> 人工智慧具有解決諸多艱鉅問題的巨大潛力，包括推進醫學發展、理解語言和促進科學發現。要實現這一潛力，負責任地使用人工智慧至關重要。為此，Google 制定了指導 Google 人工智慧應用的原則、與 Google 外部社群分享我們工作的最佳實務，以及使我們的努力合理化的計畫。這些原則指出，人工智慧應具有社會效益、避免造成或強化不公平的偏見、以安全為重並進行測試、對人們負責、納入隱私設計原則、堅持高水平的科學卓越，並僅用於符合這些原則的用途。此外，Google 不會在以下應用領域設計或佈署人工智慧：可能造成整體傷害的技術；武器或其他其主要目的或實施方式為造成或直接促成人員傷害的技術；收集或使用資訊進行監控、違反國際公認規範的技術；以及其目的違反廣泛接受的國際法和人權原則的技術。儘管這些是 Google 自身的 AI 指導原則，但我們也敦促其他組織制定自身的一套原則，以鼓勵負責任的 AI 開發。讓組織除錯和改進機器學習模型的效能，並幫助他人理解其模型的行為也很重要。建立機器學習模型的組織也需要協助偵測和解決其數據和模型中的偏差、漂移和其他差距。此外，提供人類可理解的機器學習模型說明將有助於提升最終用戶的信任度和透明度。可解釋人工智慧是 Google Cloud 的一套工具和框架，可幫助您理解和解釋機器學習模型做出的預測。這些工具與多種 Google 產品和服務原生整合，以確保人工智慧開發的透明度。
...
* Google Cloud 提供四種機器學習模型建構方案：
    ...
    * 自訂訓練：提供高度彈性及掌控權，讓使用者自行編寫及佈署 ML 環境。
...
> 大型數據集的機器學習需要大量的程式設計和機器學習框架的知識。這些要求將解決方案的開發限制在每個公司內非常小的一群人，並且排除了那些理解數據但機器學習知識和程式設計專業知識有限的數據分析師。儘管 BigQuery 最初僅作為資料倉儲，但隨著時間的推移，它已經發展出更多支持數據到 AI 生命週期的功能。BigQuery ML 通過讓數據分析師（主要是資料倉儲用戶）能夠使用現有的商業智能工具和電子表格來構建和運行模型，從而使機器學習的使用民主化。預測分析可以指導整個組織的商業決策。無需使用 Python 或 Java 程式設計 ML 解決方案。模型直接在 BigQuery 中使用 SQL 進行訓練和訪問，SQL 是一種數據分析師熟悉的語言。BigQuery ML 將機器學習帶入數據中。它降低了複雜性，因為需要的工具更少。它還提高了生產速度，因為不需要為 BigQuery 中的模型訓練移動和格式化大量數據以用於基於 Python 的 ML 框架。BigQuery ML 還與 Vertex AI 集成，Vertex AI 是 Google Cloud 端到端 AI 和 ML 平台。當 BigQuery ML 模型註冊到 Vertex AI 模型註冊表時，它們可以佈署到端點以進行線上預測。
...
*   BigQuery ML 整合 Vertex AI，方便模型佈署至端點進行線上預測。
...
> Google Cloud 的預訓練 API 是一個絕佳的選擇，尤其當您沒有自己的訓練數據時。這非常適合那些沒有專門資料科學家，但擁有商業分析師和開發人員的組織。這是機器學習方法中最快速、最省力的方式，但可定制性不如其他方法。Google Cloud 的預訓練 API 能透過提供用於分析圖像、影片和文字等常見任務的機器學習模型，幫助開發人員快速構建智能應用程式。無論機器學習經驗水平如何，API 都可以佈署在虛擬私有雲、本地或 Google 的公有雲中。讓我們想像一下，一位開發人員正在構建一個行動應用程式，使用者可以提交照片給開發人員，應用程式需要識別圖像內容並過濾掉任何不適合工作的圖片。開發人員可能會選擇 Vision API。它提供強大的預訓練機器學習模型，使用 Google 數據自動檢測圖像中的面部、物體、文字甚至情感。開發人員可以使用 Vision API 為圖像分配標籤，並將其快速分類到數百萬個預定義類別中。自然語言 API 是另一個開箱即用的預訓練 API。如果一家企業在其網站上有一個每天接收大量訊息的聯繫表單，這些數據可能難以且耗時地進行人工處理、分類和執行操作。自然語言 API 能夠發現文字中的語法、實體和情感，並將文字分類到一組預定義的類別中。在本例中，它可以判斷評論是否代表抱怨、讚美，並嘗試進一步理解您的業務。Google 還提供其他幾個預訓練 API。Cloud Translation API 可將文字從一種語言翻譯成另一種語言。語音轉文字 API 可將音訊轉換為文字以進行數據處理。文字轉語音 API 可將文字轉換為高品質的語音音訊。而影片智慧 API 則可以識別影片中的動作和行為。機器學習模型的訓練效果取決於可用於訓練它的數據量。正如您可能預期的那樣，Google 擁有大量的圖像、文字和機器學習研究人員來訓練其預訓練模型。這意味著您可以減少工作量，並更快地獲得投資回報。


##### 摘要

* Google Cloud 預訓練API適用於缺乏自身訓練數據的使用者，尤其適合擁有商業分析師和開發人員，但缺乏資料科學家的組織。
* 此方法快速、省力，但客製化程度較低。
* 可快速建構智慧型應用程式，分析圖像、影片和文字。
* 佈署彈性高，支援虛擬私有雲、本地和 Google 公有雲。
...
> Vertex AI 也是建立自訂端到端機器學習模型的重要平台。這意味著不僅可以使用您自己的數據訓練模型，而且模型也是自訂建立的。Vertex AI 提供一套產品，可在機器學習工作流程的每個階段提供幫助，從收集數據到未來的工程、建立模型，最後到佈署和監控這些模型。由於這種方法是完全自訂建立的，端到端的，因此它的過程最長，需要一個專業的資料科學家和工程師團隊。但是，這些完全自訂的機器學習模型最能滿足您的需求，並為您的業務帶來最大的差異化和創新成果。Vertex AI 包含可協助程式設計師進行虛擬機器映像、數據標記、訓練和預測的工具。它還提供預建演算法。重要的是要記住，儘管這些工具是每個階段使用您的數據的構建塊，但沒有任何一種方法適合所有情況。每個用例都需要不同的工具和產品組合。


##### 摘要

*   Vertex AI 提供自訂端到端機器學習模型建立平台。
*   涵蓋機器學習工作流程所有階段：數據收集、模型建立、佈署及監控。
*   完全客製化，需專業資料科學家及工程師團隊，耗時較長。
*   模型高度專精，帶來最大差異化及創新成果。
...
所有機器學習模型均建構於 Google Cloud 的 AI 基礎架構之上。此基礎架構的一部分是 TensorFlow，這是一個端到端的開放原始碼機器學習平台。TensorFlow 擁有靈活的工具、函式庫和社群資源生態系統，讓研究人員能夠在機器學習領域創新，並讓開發人員能夠建置和佈署搭載機器學習的應用程式。TensorFlow 最初是為 Google 內部使用而開發，現在已成為開放原始碼，以便讓所有人受益。TensorFlow 利用 Tensor Processing Unit (TPU)，這是 Google 專門開發的應用程式特定積體電路，用於加速機器學習工作負載。TPU 作為特定領域的硬體，而非像 CPU 和 GPU 那樣的通用型硬體。透過 TPU，運算速度提升超過 200 倍。這表示，使用最先進的 GPU 需等待 26 小時才能獲得結果，而使用完整的雲端 TPU 叢集則只需等待 7.9 分鐘即可獲得相同的結果。雲端 TPU 已整合到 Google 的多項產品中，此最先進的硬體和超級運算技術可透過 Google Cloud 產品和服務取得。
...
> 除了可自訂選項外，Google Cloud 也創造了一系列完整的 AI 解決方案，旨在解決特定的業務需求。聯繫中心 AI 提供與客戶交談和協助人工座席的模型，提高營運效率，並個性化客戶服務，從而轉型您的聯繫中心。文件 AI 通過提取和分類來自非結構化文件（例如發票、收據、表格、信函和報告）的信息來解鎖見解。然後可以將提取的數據保存到資料庫中或導出到另一個應用程序以進行進一步分析。零售發現 AI 使用機器學習來選擇零售商電子商務網站上產品的最佳排序，當購物者選擇冬季夾克或廚房用品等類別時。隨著時間的推移，AI 通過使用歷史數據來學習每個頁面的理想產品排序，優化產品的展示方式和內容，以提高準確性、相關性和銷售可能性。而 Cloud Talent Solution 利用 AI 進行求職和人才招聘，更快地將候選人與理想工作匹配，並讓雇主吸引和轉化更高質量的候選人。這些只是 Google Cloud 提供的一些完整的 AI 解決方案。
...
> Google Cloud 提供一系列 AI 和 ML 解決方案及產品，但在選擇使用哪一種方案時，需要考慮幾項決策和權衡。首先是速度：您需要多快才能將模型投入生產？AI 專案的規劃和實施通常需要 3 到 36 個月，具體取決於用例的範圍和複雜性。但業務決策者往往低估了所需的時間。預訓練 API 無需模型訓練，因為這項耗時的任務已經完成。自訂訓練通常所需時間最長，因為它從頭開始構建 ML 模型，這與 AutoML 和 BigQuery ML 不同。接下來是差異化：您的模型有多獨特，或者它需要多麼獨特？Google Cloud 為希望快速在其日常業務運營中使用 ML 模型的組織提供一系列現成解決方案。這些包括圖像識別解決方案和聊天機器人，它們佈署迅速，並可應用於各種用例。或者，Vertex AI 作為 Google Cloud 的統一平台，可用於構建、佈署和管理 AI 解決方案，可以讓 ML 工程師和資料科學家完全控制 ML 工作流程。Vertex AI 自訂訓練讓您可以使用 Vertex Workbench 上的代碼訓練和服務自訂模型，從而產生高度定制的 ML 模型。另一個考慮因素是在開始 AI 或 ML 專案時所需的專業知識。將 AI 整合到業務流程中需要資料工程師、資料科學家和機器學習工程師等角色。組織應考慮其目前的團隊，然後確定人員策略，這可能包括重複使用或調整現有資源、提升和培訓現有員工，或聘用或與外部顧問或承包商合作。Google Cloud 的 AI 和 ML 產品種類繁多，從數據分析師和商業情報團隊可以使用產品，到更適合 ML 工程師和資料科學家的產品，應有盡有。最後一個考慮因素是構建 AI 解決方案所需的工作量。這取決於幾個因素，包括問題的複雜性、可用數據量和團隊的經驗。Google Cloud 可以幫助提供適用於規模兩端的專案的解決方案。但是，任何 AI 工作通常都需要大量時間、精力和專業知識才能對業務運營產生有價值的影響。
...
*   **專業知識:**  需要資料工程師、資料科學家、機器學習工程師等角色，需考量團隊現狀及人員策略。
...
> 服務基礎設施 (Infrastructure as a Service, IaaS) 允許使用者透過虛擬機器虛擬化硬體來與其他開發者共享計算資源。這讓每個開發者都能在其自包含的環境中佈署自己的作業系統、存取硬體並構建應用程式，同時擁有必要的系統資源。容器遵循與虛擬機器相同的原則，它們提供隔離的環境來執行軟體服務並優化單個硬體的資源，但效率更高。虛擬機器與容器的主要區別在於，虛擬機器將整個機器虛擬化到硬體層，而容器只虛擬化作業系統層以上的軟體層。容器啟動速度更快，與啟動整個作業系統相比，使用的記憶體也少得多。容器與您的應用程式及其所有依賴項一起打包，因此它擁有運行所需的一切。容器可以獨立開發、測試和佈署，非常適合基於微服務的架構。這種架構由運行容器化應用程式的較小的個別服務組成，這些服務通過 API 或其他輕量級通訊方法（例如 REST 或 gRPC）相互通訊。容器讓開發者可以建立與其他系統資源隔離的可預測環境。因此，如果客戶要求新的功能或應用程式的更改，開發者可以輕鬆地更新應用程式的特定部分，而不會影響其餘部分。容器幾乎可以在任何地方運行，這使得開發和佈署變得容易。
...
*   容器適用於微服務架構，可獨立開發、測試和佈署。
*   容器提供隔離的、可預測的環境，方便更新和佈署。
...
> 容器提升敏捷性、增強安全性、優化資源並簡化雲端應用程式的管理。許多組織同時使用虛擬機器和容器。然而，隨著 IT 基礎架構變得越來越複雜，他們常常需要一種方法來管理其服務和機器。例如，一個組織可能有數百萬甚至數千萬個容器，這需要大量的監督和管理才能確保其安全有效地運作。Kubernetes 是一個由 Google 開發的開源平台，用於管理容器化工作負載和服務。它可以輕鬆地在許多主機上編排許多容器，擴展它們，並輕鬆佈署和回滾。這提高了應用程式的可靠性，並減少了管理和運營所需的時間和資源。Google Kubernetes Engine (GKE) 是 Google 在雲端託管的受管理 Kubernetes 服務。GKE 環境包含多台機器，具體來說是組成叢集的計算引擎實例。GKE 叢集可以自定義，並且支援不同的機器類型、節點數量和網路設定。GKE 通過提供 API 和基於 Web 的主控台，簡化了應用程式的佈署。應用程式可以在幾分鐘內佈署，並可以根據需要擴展或縮減。GKE 還提供許多功能，可以幫助監控應用程式、管理資源和排除問題。讓我們探討一下日本醫療科技新創公司 Ubie 如何利用 Google Kubernetes Engine 降低其基礎架構成本和維護需求。Ubie 成立於 2017 年，其目標是在人們需要時為他們提供正確的醫療護理，它通過為醫院和個人設計的產品實現這一目標。Ubie for Hospital 是他們的旗艦產品，是一款基於 AI 的問卷軟體，讓患者可以在預約前提供醫療詳情。Ubie 最初依賴於另一個雲平台在日本提供 Ubie for Hospital。隨著業務增加新的客戶，他們需要一個能夠支援每日佈署並提供安全網關以將 Ubie 連接到各種客戶網路和設定的基礎架構。Ubie 評估了可用的選項，並決定使用 Google Kubernetes Engine 中的 Kubernetes。Google Kubernetes Engine Autopilot 是一種模式，它可以全面管理整個叢集的基礎架構並提供按 Pod 計費，為業務更有效率且經濟地運行 Ubie for Hospital 提供了一個引人注目的選項。使用 GKE Autopilot，Ubie 可以消除配置和監控叢集的需要，而只需為運行的 Pod 付費。這一轉變使 Ubie 的基礎架構成本降低了 20%，而 GKE Autopilot 幫助該公司消除了 Ubie for Hospital 基礎架構維護和升級任務，這些任務可能需要數小時甚至數天才能完成。在 Google Cloud 上運行容器化應用程式的另一個熱門選項是 Cloud Run。Cloud Run 是一個完全託管的無伺服器平台，用於佈署和運行容器化應用程式，而無需擔心底層基礎架構。在您的應用程式程式碼容器化並佈署到 Cloud Run 後，Google Cloud 會自動負責擴展和管理基礎架構。Cloud Run 非常適合運行需要快速根據流量擴展和縮減的無狀態應用程式。這使得 Cloud Run 最適合簡單輕量級的應用程式，例如 Web 應用程式。總而言之，當需要對 Kubernetes 環境進行大量控制並且需要運行複雜的應用程式時，GKE 是理想的選擇。或者，當需要一個簡單、完全託管的無伺服器平台並且可以快速擴展和縮減時，Cloud Run 是理想的選擇。
...
*   Mashme.io 案例：使用 Google Kubernetes Engine 實現低延遲、全球覆蓋和持續佈署的教育科技視頻協作平台。
...
> 雲端科技的進步徹底改變了軟體應用的開發方式。現代雲端應用程式開發具有彈性、可擴展性，並利用最新的雲端運算技術來建構和佈署應用程式。過去，傳統的軟體開發方法，通常稱為單體應用程式，需要將應用程式的所有組件作為單個、緊密耦合的單元進行開發和佈署，通常使用單一程式語言。現代雲端應用程式開發方法有很多優點。讓我們探討其中幾點。我們將從架構開始。現代雲端應用程式通常建構為微服務的集合。微服務是可以獨立佈署、擴展和維護的組件，可用於建構各種應用程式。這可以幫助組織更快地將商業價值推向市場，因為功能可以在完成後立即發布，而無需等待應用程式的其餘部分完成。關於佈署，現代應用程式通常佈署到雲端，並且可以使用託管或部分託管的服務。託管服務負責雲端基礎架構的日常管理，例如修補程式、升級和監控。這可以讓員工騰出時間專注於其他任務，例如開發新應用程式。部分託管服務提供混合方法，企業自行管理其基於雲端的應用程式的某些方面，而雲端提供商則管理其他方面。在成本方面，現代雲端應用程式使用按需付費的定價模式，在有效配置的情況下，這可以使其極具成本效益。這意味著組織並不總是需要為他們沒有充分利用的資源付費。開發人員還可以利用預建 API（我們稍後將在本課程的這一部分中探討）和雲端提供商提供的其他工具來更快地建構和佈署其應用程式。然後是可擴展性。現代基於雲端的應用程式可以輕鬆地向上或向下擴展以滿足用戶需求。現代雲端應用程式旨在具有高可用性和彈性，並具有負載平衡（將網路流量均勻分佈到支援應用程式的多個伺服器上的過程）和自動故障轉移（允許基於雲端的應用程式在發生故障時自動切換到備份伺服器上的過程）等內建功能。此外，雲端服務提供商通常提供強大的監控和管理工具，允許開發人員快速識別和響應問題，這可以進一步提高雲端應用的可靠性。


##### 摘要

*   **架構:**  採用微服務架構，獨立佈署、擴展及維護，加快產品上市速度。
*   **佈署:**  雲端佈署，提供託管及部分託管服務，降低管理負擔。
*   **成本:**  按需付費模式，提高成本效益。
...
> 企業導入API後，有效維護和管理至關重要。可以使用Apigee API管理平台（Google Cloud的API管理服務）來操作API，以增強規模、安全性及自動化。Apigee是許多組織管理API的熱門選擇，因为它具有許多優點，例如：提供身份驗證、授權和數據加密等功能來保護API；透過即時分析和歷史報告追蹤和分析API使用情況；透過視覺化API編輯器和測試沙盒來協助開發和佈署API；提供API版本控制、API文檔，甚至API節流（限制用戶在特定時間段內可以發送的API請求數）。AccuWeather透過API與全球合作夥伴共享其世界級的天氣數據，這些合作夥伴開發了適用於互聯汽車、智慧家居、穿戴式設備、智慧電視、行動裝置等的應用程式，獲得了巨大的成功。但公司希望將數據提供給新的客戶：個別開發人員。它需要一種方法來吸引這些用戶，並根據開發人員的不同需求調整其產品，並據此將不同的產品級別貨幣化。為實現讓開發人員以簡單快速的方式開始構建，並根據其需求提供適當級別的API呼叫和功能，AccuWeather意識到需要一個精密的API管理平台。該平台能夠通過將API捆綁到不同的產品中來提供不同層次的產品，每個產品都有自己的速率限制和定價。借助Apigee管理API，AccuWeather的用戶可以根據自身需求自定義API使用情況，而Apigee則有助於吸引和建立流量。借助可自定義的Apigee開發人員入口網站，開發人員可以快速註冊、理解AccuWeather API並測試它們。借助內建分析功能，AccuWeather可以密切關注註冊用戶、他們產生的流量及其來源，並觀察流量活動中的意外模式。
...
*   Apigee優點包括：API安全防護、使用情況追蹤分析、開發佈署支援、版本控制及節流功能。
...
> 雲端安全領域中，理解相關術語對於有效掌握整體環境至關重要。本課將介紹討論雲端安全時常見的基本安全術語和概念。讓我們來探討這些術語及其重要性。前三個概念與降低未經授權存取敏感資料的風險有關。特權存取安全模型允許特定使用者存取比一般使用者更廣泛的資源集。例如，系統管理員可能擁有特權存取權限以執行疑難排解和資料還原等任務。然而，濫用特權存取權限可能會造成風險，因此務必仔細管理和監控此類存取權限。最小權限安全原則提倡僅授予使用者執行其工作職責所需的存取權限。透過提供最低限度的必要存取權限，組織可以降低未經授權存取敏感資料的風險。例如，銷售代表可能只需要存取客戶關係管理 (CRM) 系統，而無需存取薪資或財務等其他系統。零信任架構安全模型假設預設情況下不信任任何使用者或裝置。每個使用者和裝置都必須經過驗證和授權才能存取資源。零信任架構透過實施嚴格的存取控制和持續驗證使用者身分，有助於確保穩健的安全性。接下來三個概念與組織如何防範網路威脅有關。預設安全性是一項原則，強調從開發的初始階段就在系統和應用程式中整合安全措施。透過在整個過程中優先考慮安全性，組織可以在其雲端環境中建立穩固的安全基礎。安全態勢是指雲端環境的整體安全狀態。它透過評估其安全控制、政策和實務，來指示組織防禦網路攻擊的準備程度。網路韌性是指組織承受並快速從網路攻擊中復原的能力。這包括識別、評估和減輕風險，有效回應事件以及快速從中斷中復原。最後，讓我們來探討保護雲端資源免於未經授權存取的基本安全措施。防火牆是一種根據預定義安全規則來調節網路流量的網路裝置。您可以將防火牆想像成網路的安全警衛。它遵循某些規則來決定允許哪些流量進入或離開網路。這些規則有助於防止未經授權的人員或有害的事物遠離重要的雲端資源，例如伺服器、資料庫和應用程式。延續之前的比喻，安全警衛會檢查所有想要進入的人，只允許有權限的人進入。同樣地，防火牆會檢查網路中的進出流量，只允許安全且經授權的流量通過。加密是使用加密演算法將資料轉換為不可讀格式的過程。然而，解密是使用加密金鑰將加密資料還原為其原始格式的反向過程。保護加密金鑰至關重要，因為它持有解密資料所需的秘密演算法。另一種思考加密和解密的方法是撰寫只有您和您想要傳送的人才能理解的秘密語言訊息。這樣，即使有人攔截訊息，他們也無法閱讀，因為他們不知道秘密語言。
...
* 理解資料加密在保護資料安全，以及應對不同風險狀態資料暴露的機制。
* 釐清驗證、授權和稽核的差異。
...
> SecOps，簡稱安全運營，旨在保護組織在雲端的數據和系統。它結合了多種流程和技術，有助於降低數據洩露、系統中斷和其他安全事件的風險。可以把它想像成保護寶貴數據的秘密武器。讓我們來探討 SecOps 中一些重要的活動。漏洞管理是識別和修復雲基礎架構和應用程式中安全漏洞的過程，如同定期檢查城堡城牆是否有薄弱之處。Google Cloud 的安全指揮中心 (SCC) 提供了安全態勢的集中視圖，有助於識別和修復漏洞，確保基礎架構穩固且受到保護。另一個關鍵活動是日誌管理，如同在城堡 grounds 上密切關注，尋找任何可疑活動。Google Cloud 提供 Cloud Logging 服務，用於收集和分析來自整個 Google Cloud 環境的安全日誌，幫助檢測和響應任何問題跡象，並預料潛在威脅。當然，為安全事件做好準備同樣重要，這就是事件響應發揮作用的地方。想像一下，有一支隨時準備保衛城堡的騎士隊伍。Google Cloud 擁有跨不同領域的專業事件響應人員，他們配備了應對任何安全事件所需的知識和工具，可以迅速有效地解決問題。SecOps 的另一個重要方面是教育員工理解安全最佳實務。就像教導城堡裡的每個人都要保持警惕並鎖好大門一樣，安全意識培訓通過提高意識和賦能員工來保護自己和組織，從而幫助預防事故。現在，您可能想知道，為什麼您的組織應該實施 SecOps？以下是優點：降低數據洩露風險：SecOps 有助於識別和修復漏洞，從而顯著降低數據洩露的風險。提高正常運行時間：迅速有效的事件響應最大限度地減少了中斷對業務運營的影響，確保服務更順暢、不間斷。增強合規性：SecOps 有助於滿足安全法規，例如通用數據保護條例 (GDPR)，並保持組織的良好聲譽。提高員工生產力：通過教育員工理解安全最佳實務，SecOps 最大限度地減少了人為錯誤的風險，並促進了更安全、更高效的工作環境。SecOps 是組織安全策略中不可或缺的一部分。通過實施 SecOps 實務，您可以加強防禦、降低安全風險並在不斷變化的雲安全環境中保護您的數據。
...
> 在 Google，我們深知隱私在贏得和維持信任方面至關重要。客戶需要確信其數據和應用程式安全可靠，因此 Google Cloud 擁有強大的信任原則和合規計劃，旨在保護客戶數據並滿足從小型企業到大型企業等廣泛客戶的需求。在本課程的最後一部分，您將理解 Google 的七大信任原則、Google Cloud 的數據駐留和數據主權選項，以及 Google Cloud 合規資源中心和合規報告管理器如何支持行業和區域合規需求。
...
> 今日數位環境下，無論規模大小的組織都擁抱雲端的力量與彈性以轉型營運模式。然而，有效管理和擴展雲端資源可能是一項複雜的任務。這就是雲端運維發揮作用的地方。雲端運維是指為確保雲端系統順利運作、優化和擴展而採用的一套實務和策略。它包含管理和監控在雲端運行的基礎架構、應用程式和服務，同時遵守可靠性、效能、安全性和成本優化的最佳實務。雲端運維在協助組織達成數位轉型目標方面扮演著關鍵角色，因為它確保了關鍵系統的可用性、效率和彈性。因此，基於此，讓我們來探討本課程的目標。「與 Google Cloud 運維一起擴展」旨在幫助您學習 Google Cloud 如何支援組織透過財務治理控制其雲端成本，理解雲端中現代運維、可靠性和彈性的基本概念，並探討 Google Cloud 如何減少我們的環境影響並幫助組織達成永續發展目標。在整個課程中，您將參加分級的知識評估。您必須通過這些評估才能獲得課程學分。好的，讓我們開始吧！
...
* 本課程目標：學習 Google Cloud 如何控制雲端成本、理解現代運維、可靠性和彈性基本概念，以及 Google Cloud 如何減少環境影響並達成永續發展目標。
* 需通過課程評估才能獲得學分。
...
> 讓我們探索一些雲端金融治理最佳實務，組織可以採納這些實務來提高其雲端資源的可預測性和控制性。第一個最佳實務是識別誰負責管理雲端成本。如果是一個團隊，理想情況下應該是由 IT 經理和財務主管混合組成。由於雲端支出分散且變動性大，因此在整個組織中建立成本責任文化非常重要。為專案定義明確的所有權，並與使用雲端資源的部門和團隊共享成本視圖，有助於建立這種責任文化和更負責任的支出。除了讓團隊對其支出負責外，Google Cloud 財務治理政策和權限也讓您輕鬆控制誰可以支出和查看整個組織的成本。此外，Google Cloud 提供靈活的選項來組織資源並將成本分配給個別部門和團隊。例如，預算會根據您實際或預測的雲端成本通知主要利益相關者。建立具有意義警示的多個預算，是掌握雲端成本的重要實務。第二個最佳實務是理解發票中可以找到哪些資訊，以及成本管理工具。它們不是相同的概念。發票是雲端服務供應商發送給客戶的文件，要求支付已使用服務的費用。但是，成本管理工具是幫助追蹤、分析和優化雲端支出的軟體。組織很少只關心他們花了多少錢。他們還想知道為什麼他們花了這麼多錢。成本管理工具（例如 Google Cloud 主控台中內建的工具）可以有效地回答「為什麼」。它們可以提供詳細的數據、發現趨勢並找出可以控制或優化成本的措施。這就帶我們到第三個最佳實務，用於提高雲端資源的可預測性和控制性：使用 Google Cloud 的成本管理工具。Google Cloud 相信透過提供強大的財務治理工具來支援組織，讓客戶更容易將其策略重點與雲端使用情況保持一致。在組織優化其雲端成本之前，他們首先需要理解他們正在花費多少錢、是否存在任何趨勢以及他們的預測成本是多少。那麼，這該如何做到呢？首先，掌握正在使用哪些雲端資源、由誰使用、用於什麼目的以及成本是多少。從那裡，確定誰將負責監控該資訊，誰將參與成本管理，以及如何持續報告支出資訊。設定與主要雲端利益相關者的持續溝通節奏和格式也很重要。事先制定此計畫有助於確保成本管理不是事後才考慮的問題。那麼，如何監控目前的成本趨勢並找出可以改進的浪費領域呢？Google Cloud 提供內建的報告功能，可以幫助您的團隊深入理解成本。理想情況下，至少應每週檢閱報告一次。一個強大的工具是 Google Cloud 定價計算器。定價計算器讓您可以估計雲端使用情況的變化將如何影響成本。計算器可在 cloud.google.com/products/calculator 找到。現在您已經有機會探索一些雲端財務治理最佳實務，下一步就是實施它們。如果這不屬於您的職責範圍，請務必將這些最佳實務傳達給組織中的相關利益相關者。
...
> 今日數位環境瞬息萬變，組織越來越多地利用雲端技術來推動創新、敏捷性和效率。然而，要真正發揮雲端的力量，需要全面理解大規模運作卓越和可靠性。運作卓越和可靠性是指組織優化其運作並確保不間斷服務交付的能力，即使它們處理日益增長的雲端工作負載和複雜性也是如此。這包括設計穩健的基礎架構、建立彈性的流程以及採用主動監控和響應機制。想像一下，一個全球電子商務平台在大型促銷活動期間經歷了突然的流量激增。為滿足增加的需求，該平台需要快速擴展其資源，同時確保不間斷的服務可用性。此處的運作卓越涉及有效擴展底層基礎架構、自動化資源配置和實施負載平衡機制。可靠性側重於最大限度地減少停機時間、採用容錯系統和實施災難恢復策略。通過在這些領域取得卓越成就，電子商務平台可以無縫處理增加的負載，提供始終如一的積極用戶體驗，並避免收入損失或聲譽損害。在本課程的這一部分，您將探討使用 Google Cloud 現代化運作、設計彈性基礎架構和流程、雲端可靠性的基本原理、Google Cloud 客戶服務以及支援案例的生命週期。
...
> 開發人員負責編寫系統和應用程式的程式碼，而操作人員則負責確保這些系統和應用程式可靠地運作。開發人員需要敏捷，常常被要求快速編寫和佈署程式碼，目標是頻繁發佈新功能、通過新功能提升核心業務價值，並快速發佈修復程式以提升整體用戶體驗。相比之下，操作人員需要保持系統穩定，因此他們通常更偏好較慢的工作節奏以確保可靠性和一致性。傳統上，開發人員將其程式碼交付給操作人員，而操作人員通常對程式碼如何在生產或線上環境中執行知之甚少。當出現問題時，兩個團隊都很難快速找出問題的根源並解決它。更糟糕的是，團隊之間的責任歸屬並不總是清晰的。DevOps 是一種軟體開發方法，它強調開發和運營團隊之間的協作和溝通，以提高軟體交付的效率、速度和可靠性。它旨在打破這些團隊之間的隔閡，並培養共享責任、自動化和持續改進的文化。DevOps 架構中的一個特定概念是網站可靠性工程 (SRE)，它確保在雲端佈署的軟體系統和服務的可靠性、可用性和效率。SRE 結合軟體工程和運營的方面來設計、構建和維護可擴展且可靠的基礎設施。監控是產品可靠性的基礎。它揭示了需要緊急關注的事項，並顯示應用程式使用模式的趨勢，這可以產生更好的容量規劃，並通常有助於改善應用程式客戶的體驗並減輕他們的痛苦。「四大黃金指標」用於衡量系統的性能和可靠性，分別是延遲、流量、飽和度和錯誤。延遲衡量系統的特定部分返回結果所需的時間。延遲很重要，因為它直接影響用戶體驗，變化可能表明出現問題，其值可能與容量需求相關，並且可用於衡量系統改進。流量衡量到達系統的請求數量。流量很重要，因為它是當前系統需求的指標，其歷史趨勢用於容量規劃，並且它是計算基礎設施支出時的核心指標。飽和度衡量系統距離容量還有多遠。但是，需要注意的是，容量通常是一個主觀的度量，取決於底層服務或應用程式。飽和度很重要，因為它是服務滿負荷程度的指標，它側重於約束最嚴格的資源，並且它經常與達到容量時的性能下降相關。錯誤是衡量系統故障或其他問題的事件。當電腦程式或系統中的缺陷、故障或錯誤導致其產生不正確或意外的結果或以意外的方式運行時，通常會引發錯誤。錯誤很重要，因為它們可能表明某些東西正在發生故障、配置或容量問題、服務級別目標違規或該發出警報了。網站可靠性工程中的三個主要概念是服務級別指標 (SLI)、服務級別目標 (SLO) 和服務級別協議 (SLA)。它們都是為系統的「四大黃金指標」設定的目標類型。服務級別指標是顯示系統或服務運行狀況的度量。它們是具體的指標，例如響應時間、錯誤率或正常運行時間百分比（即系統可用的時間量），這些指標有助於我們理解系統的行為和性能。服務級別目標是我們根據 SLI 為系統性能設定的目標。它們定義了我們想要達到的可靠性或性能水平。例如，SLO 可能規定系統在一個月內應有 99.9% 的時間可用。服務級別協議是雲服務提供商及其客戶之間的協議。它們概述了關於服務質量的承諾和保證。SLA 包括商定的 SLO、性能指標、正常運行時間保證以及如果提供商未能履行這些承諾的任何處罰或補救措施。這可能包括當服務的中斷時間長於此協議允許的時間時退款或信用額度。
...
> 本文探討在雲端環境中監控伺服器、資料庫或應用程式的運作情況。與本地環境不同，雲端伺服器無法直接物理接觸，因此需要借助 Google Cloud 的整合式可觀察性工具來理解系統的效能、健康狀況和行為。Google Cloud Observability 提供一套完整的監控、日誌記錄和診斷工具，包括 Cloud Monitoring、Cloud Logging、Cloud Trace、Cloud Profiler 和 Error Reporting 等服務，以實現對應用程式和基礎架構的全面監控和洞察。
...
> 本課程即將結束，在您探索雲端運算如何協助轉型業務模式後，務必強調 Google 在技術方面的努力與我們對環境和永續發展的承諾。虛擬世界，包括 Google Cloud 的網絡，是建立在物理基礎架構上的，所有這些嗡嗡作響的伺服器機架都消耗大量能源。現有資料中心總共消耗了全球近 2% 的電力。有鑑於此，Google 致力於使我們的資料中心儘可能有效率地運作。與我們的客戶一樣，Google 也試圖保護地球。我們理解 Google Cloud 客戶也有自己的環保目標，在 Google Cloud 上運行其工作負載可以成為達成這些目標的一部分。因此，值得注意的是，Google 的資料中心是首批獲得 ISO 14001 認證的資料中心，這是一項標準，概述了組織如何透過改進資源效率和減少浪費來提升其環境績效的框架。例如，Google 位於芬蘭哈米納的資料中心。該設施是 Google 機群中最先進、最高效的資料中心之一。其冷卻系統使用芬蘭灣的海水，減少能源使用，並且是全球首創。在 Google 成立的十年間，Google 成為第一家實現碳中和的主要公司。在我們的第二個十年裡，我們是第一家實現 100% 使用再生能源的公司。到 2030 年，我們的目標是成為第一家完全實現碳中和運作的主要公司。我們透過努力賦能每個人——企業、政府、非營利組織、社區和個人——利用 Google 技術創造更永續的世界，來應對氣候變化和資源效率的需求。那麼，在實踐中這看起來是什麼樣子呢？讓我們探討一個客戶 Kaluza 如何使用 Google Cloud 技術啟動智慧電動汽車充電計劃的示例，該計劃可幫助客戶省錢，同時減少他們的碳足跡。電動汽車已佔全球汽車銷量的七分之一，隨著全球逐步淘汰新的汽油和柴油汽車，預計到 2040 年全球銷量將達到 7300 萬輛。但隨著電網越來越依賴風能和太陽能等可變能源，電動汽車需求的增加有可能在高峰時間過度消耗電網，從而可能導致停電。Kaluza 於 2019 年由 OVO Energy 推出，它利用其對能源市場的深入理解，與全球一些主要的能源供應商和汽車製造商合作。透過名為「隨時充電」的計劃，客戶使用 Kaluza 智慧充電他們的電動汽車，他們只需支付約三分之一的家庭用電費。這意味著，如果客戶在放工回家後（例如下午 6:00）插入汽車充電——此時電網的需求和碳強度最高——他們的汽車將在夜間以最低成本和最環保的時間段智慧充電，這樣就可以在他們早上需要時準備好。Kaluza 智慧充電解決方案的背後是一些先進的技術，所有這些技術都是建立在 Google Cloud 之上的。他們的核心優化引擎從各種來源收集實時數據，包括電動汽車的電池和充電數據，以及能源供應商和電網運營商的數據，例如碳強度和價格預測。這些數據儲存在 BigQuery 中，用於訓練和驗證智慧充電優化模型。然後，這些模型將與 Google Kubernetes Engine 一起佈署，以便每當客戶插入電動汽車時，來自該汽車的數據都會通過其優化引擎實時傳遞，以計算該汽車的理想充電時間表，從而確保它使用最便宜、碳排放量最低的可用能源。至於電網運營商和能源公司，Kaluza 平台讓他們可以隨時查看有多少參與的電動汽車已插入網絡。BigQuery 和 Looker Studio 儀表板提供了詳細的見解，例如有多少汽車閒置、有多少汽車正在充電以及我們的優化引擎的工作情況。在 Google，我們始終致力於永續發展，並繼續領導和鼓勵像 Kaluza 這樣的其他人加入我們，共同改善我們地球的健康狀況。
...
* **進階學習:**  建議瀏覽 cloud.google.com/training 理解更多 Google Cloud 產品與服務。
